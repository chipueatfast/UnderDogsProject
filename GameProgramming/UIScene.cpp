...
void UIScene::CalculateHealthBar(int health)
{
	_healthRect.top = 0;
...
UIScene::UIScene()
{
	...
	_healthLamp->setSprite(new Sprite("Res\\HealthLamp.png", 246, 66));
	_healthLamp->set_state_manager(new StateManager("Res\\HealthLampXML.xml"));
	_healthLamp->state_manager()->AddToDictState("0", "Idle");
	_healthLamp->state_manager()->setState("0");
...
void UIScene::Render(int health, int UP, int appleCount, int diamondCount, int score)
{
	...
	_healthLamp->Next();
	_healthLamp->setScale(D3DXVECTOR2(1, 1));
